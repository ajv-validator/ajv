...
function getPathExpr (currentPath, expr, jsonPointers) {
  ...
  if (jsonPointers) {
    value = '\'/\' + String(' + expr + ').replace(/~/g, \'~0\').replace(/\\//g, \'~1\')';
  } else {
    value = '\'[\\\'\' + ' + expr + ' + \'\\\']\''
  }

  return joinPaths(currentPath, value);
}
...
function getPath (currentPath, prop, jsonPointers) {
  ...
  if (jsonPointers) {
    value = toQuotedString('/' + prop.replace(/~/g, '~0').replace(/\//g, '~1'));
  } else {
    value = toQuotedString(getProperty(prop));
  }
