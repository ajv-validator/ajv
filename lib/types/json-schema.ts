...
type StrictNullChecksWrapper<Name extends string, Type> =
  undefined extends null ? `strictNullChecks must be true in tsconfig to use ${Name}` : Type
...
type UncheckedJSONSchemaType<T, IsPartial extends boolean> = (
  ...
  | ({
      type: readonly (T extends number ? JSONType<"number" | "integer", IsPartial>
    ...
    : T extends Record<string, any> ?
      {
        ...
        properties?: IsPartial extends true ? Partial<UncheckedPropertiesSchema<T>>
        ...
        true // "required" is not necessary if it's a non-partial type with no required keys // are listed it only asserts that optional cannot be listed. // "required" type does not guarantee that all required properties
      ) ?
