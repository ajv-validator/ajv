...
function Ajv(opts) {
    ...
    function compile(schema) {
        var schemaObj = _addSchema2(schema);
    ...
    function addSchema(schema, key, _skipValidation) {
        ...
        checkUnique(key);
        self._schemas[key] = _addSchema2(schema, _skipValidation);
    }
    ...
    function _addSchema(schema, skipValidation) {
        var schemaObj = _addSchema2(schema, skipValidation);
        return typeof schemaObj == 'function'
                ? schemaObj
                : _compile(schemaObj);
    }
    ...
    function _addSchema2(schema, skipValidation) {
