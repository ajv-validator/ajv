...
for (const fullDynamicRefs of [false, true]) {
  runTest({
...
function runTest({instances, draft, tests, skip = [], remotes = {}}: SchemaTest) {
  ...
  jsonSchemaTest(withStandalone(instances), {
    description: `JSON-Schema Test Suite draft-${draft}${
      instances[0].opts.fullDynamicRefs ? " (fullDynamicRefs)" : ""
    }: ${instances.length} ajv instances with different options`,
    suites: {tests},
    only: [],
    skip,
    assert: chai.assert,
